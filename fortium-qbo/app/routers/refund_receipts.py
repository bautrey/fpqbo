...
router = APIRouter(
    prefix="/refund-receipts",
    tags=["refund-receipts"],
...
async def list_refund_receipts(
    ...
    try:
        return await qbo.get_refund_receipts(company_id=company_id, max_results=max_results)
...
async def get_refund_receipt(
    ...
    try:
        result = await qbo.get_refund_receipt_by_id(company_id, entity_id)
        if not result:

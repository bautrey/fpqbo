...
async def list_sales_receipts(
    ...
    try:
        return await qbo.get_sales_receipts(company_id=company_id, max_results=max_results)
...
async def get_sales_receipt(
    ...
    try:
        result = await qbo.get_sales_receipt_by_id(company_id, entity_id)
        if not result:

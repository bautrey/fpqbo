...
router = APIRouter(
    prefix="/time-activities",
    tags=["time-activities"],
...
async def list_time_activities(
    ...
    try:
        return await qbo.get_time_activities(company_id=company_id, max_results=max_results)
...
async def get_time_activity(
    ...
    """Get a specific time activity by ID."""
    try:
        result = await qbo.get_time_activity_by_id(company_id, entity_id)
        if not result:
            raise HTTPException(status_code=404, detail="Time activity not found")
        return result
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
